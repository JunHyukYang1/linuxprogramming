...
void Shape::set(int x, int y) {       // Shape 클래스의 멤버 변수에 값을 저장하는 함수 정의
    this->x = x, this->y = y;
}

void Shape::show_S() {                              // 결과 출력을 위한 함수 show_S() 정의
    cout << "좌표 (" << x << "," << y << ")에 ";
}

void Circle::setCircle(int x, int y, int r) {       // Circle 클래스의 멤버 변수에 값을 저장하는 함수 정의
    set(x, y);                                      // 상속받은 Shape 클래스의 멤버 함수를 이용하여 x와 y에 값 저장
    this->r = r;                                    // 반지름 값 변수에 저장
}

void Circle::show() {                               // Circle 클래스의 결과 출력을 위한 멤버 함수 정의
    show_S();                                       // 상속받은 Shape 클래스의 멤버 함수를 활용
    cout << "반지름 " << r << "인 원" << endl;
}

void Rect::setRect(int x, int y, int width, int height) {    // Rect 클래스의 멤버 변수에 값을 저장하는 함수 정의
    set(x, y);                                               // 상속받은 Shape 클래스의 멤버 함수를 이용하여 x와 y에 값 저장
    this->width = width;                                     // 폭 변수에 값 저장
    this->height = height;                                   // 높이 변수에 값 저장
}

void Rect::show() {                       // Rect 쿨래스의 결과 출력을 위한 멤버 함수 정의
    show_S();                             // 상속받은 Shape 클래스의 멤버 함수를 활용
