...
public:
	...
	bool operator==(int op2);             // 객체와 정수 간 == 연산의 재정의를 위한 선언
	bool operator==(string op2);          // 객체와 문자열 간 == 연산의 재정의를 위한 선언
	bool operator==(Book& op2);           // 객체와 객체 간 == 연산의 재정의를 위한 선언
};

bool Book::operator==(int op2) {         // 객체와 정수 간 == 연산 재정의
	return this->price == op2;           // return 안의 수식이 참일 시 true 반환
}

bool Book::operator==(string op2) {      // 객체와 문자열 간 == 연산 재정의
	return this->name == op2;            // return 안의 수식이 참일 시 true 반환
}

bool Book::operator==(Book& op2) {                    // 객체와 객체 간 == 연산 재정의
	return this->name == op2.name && this->price == op2.price
		&& this->page == op2.page;              // return 안의 수식이 참일 시 true 반환
}
